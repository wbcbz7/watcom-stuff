...
void fdbuildTexture() {
    ...
    */
    ...
    for (y = -128; y < 128; y++) {
        for (x = -128; x < 128; x++) {
            fdtexture[k++] = sat(((int)(0x20000 / ((x*x ^ y*y) + ee))) - ((int)(0x20000 / ((x*x + y*y) + ee))), 255) & 0xFF;
            //texture[((y << 8) + x)] = (x ^ y) & 0xFF;
...
int main() {
    ...
    for (j = 0; j < 32; j++) for (i = 0; i < 256; i++) fdpalxlat[p++] = (i * j) >> 5;
    ...
    while (!kbhit()) {
        ...
        fdCalcPlanes((i << 3), 0, 0, 
                    ((i << 8) & 0xFFFF), ((i << 8) & 0xFFFF), (((i << 6) + (i << 7)) & 0xFFFF));
