...
void fdbuildTexture() {
    ...
    for (y = 0; y < 256; y++) {
        for (x = 0; x < 256; x++) {
            ...
            fdtexture[((y << 8) + x)] = (64 + (sintab[(x << 8) & 0xFFFF] >> 9)) + (64 + (costab[(y << 8) & 0xFFFF] >> 9));
    ...
    */
    ...
    for (y = -128; y < 128; y++) {
        for (x = -128; x < 128; x++) {
            //fdtexture[k++] = 255 - sat(((int)(0x8000 / ((x*x ^ y*y) + ee))) + ((int)(0x8000 / ((x*x + y*y) + ee))), 255);
            fdtexture[k++] = sat(((int)(0x20000 / ((x*x ^ y*y) + ee))), 255);
        ...
        for (x = 0; x < 41; x++) {
            ...
            */
            ...
            if (t >= bb) {
                ...
                z = ((unsigned short)sat(t, 65535)) >> 3;
            }
...
void fdBlend() {
    ...
    for (i = 0; i < 64000; i++) {
        ...
        *((char*)scrptr+i) = fdblendtab[(*((char*)scrptr+i) << 8) + *((char*)bufptr+i)];
...
int main() {
    ...
    for (j = 0; j < 32; j++) for (i = 0; i < 256; i++) fdpalxlat[p++] = /*255 -*/ sat((i + (i * j)) >> 5, 255); p = 0;
    for (j = 0; j < 256; j++) for (i = 0; i < 256; i++) fdblendtab[p++] = sat(((i * 128) >> 8) + ((j * 160) >> 8), 255);
   ...
   rtc_setTimer(&timer, rtc_timerRate / 60);
    ...
    while (!kbhit()) {
        ...
        fdCalcPlanes((i << 3), 0, 0, 
                    (((i << 8) + (int)(0x0000 * costabf[(i << 7) & 0xFFFF])) & 0xFFFF),
