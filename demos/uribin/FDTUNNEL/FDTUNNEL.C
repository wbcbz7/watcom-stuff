...
void fdCalcPlanes(int ax, int ay, int az, int rx, int ry, int rz) {
    ...
    for (y = 0; y < 26; y++) {
        for (x = 0; x < 41; x++) {
            ...
            origin.x = 0;// + (sintab[((x << 11) + (ax << 2)) & 0xFFFF] >> 11);
            ...
            */
            ...
            v = (unsigned int)((fabs(atan2(intersect.y, intersect.x)) * 1 * 65536 * _1pi));
...
int main() {
    ...
    for (j = 0; j < 256; j++) for (i = 0; i < 256; i++) fdpalxlat[p++] = (i * j) >> 8; p=0;
    ...
    while (!kbhit()) {
        ...
        fdCalcPlanes((i << 3), (costab[((i << 6) + (i << 8)) & 0xFFFF]) >> 9, (sintab[((i << 8) + (i << 7)) & 0xFFFF]) >> 9, 
