...
void fdCalcPlanes(int ax, int ay, int az, int rx, int ry, int rz) {
    ...
    float t, l, fx, fy, delta, a, b, c, t1, t2, d;
    float tsq, _2a;
    ...
    for (y = 0; y < 26; y++) {
        for (x = 0; x < 41; x++) {
            ...
            */
            ...
            t = (direction.y >= 0 ? (PlaneSize + origin.y) / direction.y : 0);            
            ...
            d = sqrt(sqr(intersect.x - origin.x) + sqr(intersect.z - origin.z));	    
            ...
            if ((d >= bb) || (t == 0)) {
                ...
                d = ((PlaneSize * 512) * 256) / d;
                z = ((unsigned short)sat(d, 65535)) >> 4;
            }
...
void fdBlur();
#pragma aux fdBlur =  " mov   esi, offset buffer    " \
                      " mov   ecx, 64000            " \
                      " xor   eax, eax              " \
                      " xor   ebx, ebx              " \
                      " @inner:                     " \
...
int main() {
    ...
    for (i = 0; i < 256; i++) {
        ...
        outp(0x3C9, sat(((i >> 2) + (i >> 3) + (i >> 4)), 63));
        outp(0x3C9, sat(((i >> 2) + (i >> 3)), 63)); 
        
    }
    for (j = 0; j < 16; j++) for (i = 0; i < 256; i++) fdpalxlat[p++] = sat(((i * j)) >> 4, 255); p = 0;
    for (j = 0; j < 256; j++) for (i = 0; i < 256; i++) fdblendtab[p++] = sat(((i * 255) >> 8) + ((j * 32) >> 8), 255);
    ...
    while (!kbhit()) {
        ...
        */            
        ...
        fdCalcPlanes(/*(i << 3)*/0, 0, 0,
                      (-0x2000-(int)(0x1000 * costabf[(i << 7) & 0xFFFF])) & 0xFFFF,
                      ( (int)(0x2000 * costabf[(i << 8) & 0xFFFF]) + (int)(0x8000 * sintabf[(i << 7) & 0xFFFF])) & 0xFFFF,
                      ( (int)(0x2000 * sintabf[(i << 8) & 0xFFFF])) & 0xFFFF);

        
        //fdCalcPlanes((i << 2), 0, 0, 
        //            (0x4000 + (/*(i << 8) +*/ (int)(0x4000 * costabf[(i << 7) & 0xFFFF])) & 0xFFFF),
        //            ((/*(i << 8) +*/ (int)(0x2000 * sintabf[(i << 6) & 0xFFFF])) & 0xFFFF),
        //            (0x4000 + (/*(i << 8) +*/ (int)(0x4000 * sintabf[(i << 7) & 0xFFFF])) & 0xFFFF)
        //            );
        ...
        fdBlend();
        p = abs(sintab[(i << 8) & 0xFFFF] >> 14);
        for (i = 0; i < p; i++) fdBlur();
