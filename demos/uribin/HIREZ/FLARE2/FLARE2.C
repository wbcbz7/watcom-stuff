...
void vecfill() {
    ...
    for (i = 0; i < flaresnum; i++) {
        p[i].x = (rand() % (stardist << 1)) - (stardist >> 0);
        p[i].y = (rand() % stardist) + (stardist >> 1);
        p[i].z = (rand() % (stardist << 1)) - (stardist >> 0);
    }
...
void initpal() {  
    ...
    for (i = 1; i < 256; i++) {
        int_pal[i].r = sat(2 + (i >> 2) + (i >> 5), 63);
        int_pal[i].g = sat(4 + (i >> 2) + (i >> 3), 63); 
        int_pal[i].b = sat(8 + (i >> 2) + (i >> 2), 63);
    }
}
...
void initspr() {
    ...
    for (y = -(SPR_SIZE / 2); y < (SPR_SIZE / 2); y++) {
        for (x = -(SPR_SIZE / 2); x < (SPR_SIZE / 2); x++) {
            ...
            flspr[k++] = sat(0x80 / (x*x + y*y + ee), 0x18);
        }
    }
}
...
int main() {
    ...
    while (!kbhit()) {
        ...
        if (!(i % 120)) e = 32;
        ptc_wait();
        if (e >= 0) {fadeshift(e); e-=(4*di); ptc_setpal(&pal[0]);} else ptc_setpal(&int_pal[0]);
        ...
        for (j = 0; j < flaresnum; j++) {
            vecmove     (0, (-0.3f*di) + (0.1f * costabf[((i << 3) + (j << 6)) & 0xFFFF]), 0, &p[j]);
            if (p[j].y <= (-stardist/2)) p[j].y = (stardist/2);
            pt[j] = p[j];
            
            vecmove     ((8 * sintabf[((i << 8) + (j << 7)) & 0xFFFF]) + (8 * costabf[((i << 7) + (j << 8)) & 0xFFFF]), 0, 0, &pt[j]);
            
            /*
            vecrotate   (0,
                        ((i << 6) + (int)((lut_size >> 1) * sintabf[(i << 6) & (lut_size - 1)])) & (lut_size - 1),
                        ((int)((lut_size >> 4) * costabf[(i << 7) & (lut_size - 1)])) & (lut_size - 1),
                        &pt[j]);
