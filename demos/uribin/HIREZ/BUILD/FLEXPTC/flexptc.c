...
typedef struct {
    int mode, buftype, pitch, width, height, start, pagesize;
    int hostsize, host_bpp, mode_bpp;
    int nonvga, dacwidth;
...
int ptc_close() {
    ...
    if (ptc_int.bankedptr != NULL) free((void*)ptc_int.bankedptr);
    vbe_SetMode(0x3);
    vbe_Done();
...
int ptc_open(char* caption, int width, int height, int bpp, int flags) {
    ...
    ptc_int.hostsize = ptc_int.pitch * height;
    ...
    if (ptc_int.vbeinfo.vbeVersion < 0x200) {ptc_int.nonvga = 0; ptc_int.dacwidth = 6;}
    ptc_int.buftype  = ((ptc_int.modeinfo.ModeAttributes & vbe_ATTR_LFB_Support) && !(flags & ptc_OPEN_FORCEBANKED)? 1 : 0);
    
    // a workaround for banked modes - create temp linear screen buffer
    if (!ptc_int.buftype) {
        ptc_int.bankedptr = (unsigned long)calloc(ptc_int.pagesize, 1);
        if (ptc_int.bankedptr == NULL) {
            vbe_Done();
            ptc_error(ptc_d_open, err[ptc_errOpenBankedPizdec]);
            return ptc_errOpenBankedPizdec;
        }
    } else ptc_int.bankedptr = NULL;
    
    if (!(flags & ptc_OPEN_FILLCONTEXT)) ptc_setcontext();
...
int ptc_update(void* buffer) {
    ...
    bankedbuf = ptc_int.bankedptr + ptc_int.start;
    linbuf    = ptc_int.ptr + ptc_int.start;
    conbuf    = ((ptc_int.buftype != 0) ? linbuf : bankedbuf);
    ...
    if ((pitch - (ptc_int.width * ((ptc_int.host_bpp + 7) >> 3))) == 0) {
        ...
        if ((!ptc_int.buftype) && (ptc_int.host_bpp == ptc_int.mode_bpp)) {
            ptc_lineartobanked((void*)ptc_int.ptr, (void*)buf, ptc_int.start, buf_pitch);
            return ptc_errOK;
        }
    }
    ...
    if (!ptc_int.buftype) ptc_lineartobanked((void*)ptc_int.ptr, (void*)ptc_int.bankedptr, 0, ptc_int.pagesize);
