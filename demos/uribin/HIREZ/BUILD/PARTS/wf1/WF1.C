...
#define cubedist  256
#define cubes     16
#define nodes     cubes * 8
vertex   n[nodes], nt[nodes];
vertex2d n2d[nodes];

// lines info
#define lines     cubes * 12
line     f[lines];
...
void vecload() {
    int max, phi, teta, count2, i, j, lptr = 0, dptr = 0;

    int dx, dy, dz;
    
    for (i = 0; i < cubes; i++) {
        n[lptr  ].x = -stardist; n[lptr  ].y = -stardist; n[lptr++].z = -stardist;
        n[lptr  ].x = -stardist; n[lptr  ].y =  stardist; n[lptr++].z = -stardist;
        n[lptr  ].x =  stardist; n[lptr  ].y =  stardist; n[lptr++].z = -stardist;
        n[lptr  ].x =  stardist; n[lptr  ].y = -stardist; n[lptr++].z = -stardist;
        n[lptr  ].x = -stardist; n[lptr  ].y = -stardist; n[lptr++].z =  stardist;
        ...
        dx = rand() % cubedist - (cubedist / 2);
        dy = rand() % cubedist - (cubedist / 2);
        dz = rand() % cubedist - (cubedist / 2);
...
void triload() {
    ...
    for (i = 0; i < cubes; i++) {
        f[lptr  ].a = (i * 8) + 0; f[lptr  ].b = (i * 8) + 1; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 1; f[lptr  ].b = (i * 8) + 2; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 2; f[lptr  ].b = (i * 8) + 3; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 0; f[lptr  ].b = (i * 8) + 3; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 4; f[lptr  ].b = (i * 8) + 5; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 5; f[lptr  ].b = (i * 8) + 6; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 6; f[lptr  ].b = (i * 8) + 7; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 4; f[lptr  ].b = (i * 8) + 7; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 0; f[lptr  ].b = (i * 8) + 4; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 1; f[lptr  ].b = (i * 8) + 5; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 2; f[lptr  ].b = (i * 8) + 6; f[lptr++].col = col;
        f[lptr  ].a = (i * 8) + 3; f[lptr  ].b = (i * 8) + 7; f[lptr++].col = col;
    }
}
...
int main() {
    ...
    while ((order < 0x4) && (!kbhit())) {
        ...
        //if ((e < e_bound) && ((order == 0) && (row < 32))) {e += 2*di; fadeout(e);}
        //if ((e > e_bound) && (order == 0)) {e = e_bound;}
        
        if ((order >= 0x2) && ((row & 31) == 0)) {e = 224;}
        if ((order >= 0x2) && (e > e_bound)) {e -= 4*di; fadeout(e);}
        ptc_setpal(&pal[0]);
        
        ptc_update(&buffer);
        if (mcpIdle) {mcpIdle();}
        
        dv.x = ((float)cubedist/4) * costabf[((i << 5) + (i << 6)) & 0xFFFF];
        dv.y = ((float)cubedist/4) * sintabf[((i << 5) + (i << 4)) & 0xFFFF];
        dv.z = ((float)cubedist/3) * costabf[((i << 2) + (i << 6)) & 0xFFFF];
