...
void fillgrid(int ax, int ay, int az, float scale, int rx, int ry, int rz) {
    ...
    for (y = 0; y < 22; y++) {
        for (x = 0; x < 41; x++) {
            ...
            origin.x = ax;// + (sintab[((x <<  9) + (az << 4)) & 0xFFFF] >> 9);
            origin.y = ay;// + (costab[((y << 10) + (az << 4)) & 0xFFFF] >> 9);
            origin.z = 0;
            
            direction.x = (float)((x << 4) - (X_SIZE/2)) * PlaneSize;
            direction.y = (float)((y << 4) - (Y_SIZE/2)) * PlaneSize * 1.2;
            direction.z = 0;
...
int main() {
    ...
    */
    ...
    while (!kbhit()) {
        ...
        fillgrid((int)(0x200 * sintabf[(i << 6) & 0xFFFF]),
                 (int)(0x200 * costabf[(i << 7) & 0xFFFF]),
                 (i << 3), 
                 (0.5f + 0.4f * sintabf[((i << 6) + (i << 7)) & 0xFFFF]),
                 0,
                 0,
