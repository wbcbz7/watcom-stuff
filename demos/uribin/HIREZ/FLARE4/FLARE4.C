...
#define maxflares 1024

int flaresnum = 0;

// orange flares info
vertex   p[maxflares], pt[maxflares];
vertex2d p2d[maxflares];

// cyan flares info
vertex   n[maxflares], nt[maxflares];
vertex2d n2d[maxflares];
...
void vecdraw (vertex2d *v, vertex *f, int param) {
    ...
    if ((py<(Y_SIZE - (SPR_SIZE)))&&(py>0)&&(px>0)&&(px<(X_SIZE - (SPR_SIZE)))&&(f->z < 0)) {
        ...
        scrptr = (long)&buffer + ((py << 9) + (py << 7) + px)*4;
...
void vecfill() {
    ...
    for (i = 0; i < flaresnum; i++) {
        p[i].x = stardist * (sintabf[((i << 9) - (i << 7)) & 0xFFFF]);
        p[i].y = stardist * (costabf[((i << 8) + (i << 6)) & 0xFFFF]);
        p[i].z = stardist * (sintabf[((i << 7) + (i << 9)) & 0xFFFF]);
    }
...
void flfill() {
    ...
    for (i = 0; i < flaresnum; i++) {
        n[i].x = stardist * (sintabf[((i << 8) + (i << 6)) & 0xFFFF]);
        n[i].y = stardist * (sintabf[((i << 7)) & 0xFFFF]);
        n[i].z = stardist * (costabf[((i << 8) + (i << 7)) & 0xFFFF]);
    }
...
int main() {
    ...
    while (!kbhit()) {
        ...
        flaresnum += di; if (flaresnum >= maxflares) flaresnum = maxflares; 
        
        vecfill();
        flfill();
        
        dv.x = ((float)stardist/3) * costabf[((i << 6)) & 0xFFFF];
        dv.y = ((float)stardist/3) * sintabf[((i << 5)) & 0xFFFF];
        dv.z = ((float)stardist/2) * costabf[((i << 6)) & 0xFFFF] - stardist*2 - (i >> 5);
