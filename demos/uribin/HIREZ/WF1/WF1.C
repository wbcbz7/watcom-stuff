...
void vecfill(int j) {
    ...
    for (i = 0; i < flares; i++) {
        p[i].x = (sintab[((i << 9) + (sintab[(j << 8) & 0xFFFF] >> 2) + (j << 9)) & 0xFFFF]) >> 9;
        p[i].y = (sintab[((i << 8) + (costab[(j << 9) & 0xFFFF] >> 1) + (j << 8)) & 0xFFFF]) >> 9;
        p[i].z = (costab[((i << 8) + (i << 9) + (costab[(j << 8) & 0xFFFF] >> 2)) & 0xFFFF]) >> 9;
    }
}
...
int main() {
    ...
    while (!kbhit()) {
        ...
        if (!((i ^ 0x5A5A) % 60)) {
            rxdx = (((rand() & 0x1F) - 16) * 0x20);
            rydx = (((rand() & 0x1F) - 16) * 0x20);
            rzdx = (((rand() & 0x1F) - 16) * 0x20);
        }
        ...
        for (j = 0; j < flares; j++) {
            ...
            vecrotate   ((rx) & (lut_size-1),
                        ((ry) + (int)((lut_size >> 2) * sintabf[(i << 6) & (lut_size - 1)])) & (lut_size - 1),
                        ((rz) + (int)((lut_size >> 2) * costabf[(i << 5) & (lut_size - 1)])) & (lut_size - 1),
                        &pt[j]);
            vecmove     (0,
                         0,
                         -(stardist * 1.4f) + 128 * (costabf[(i << 8) & (lut_size - 1)]), &pt[j]);
        ...
        for (j = 0; j < nodes; j++) {
            ...
            vecmove     (0,
                         0,
                         -(stardist * 1.4f) + 128 * (costabf[(i << 8) & (lut_size - 1)]), &nt[j]);
