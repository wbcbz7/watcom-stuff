...
int facesort(const void *op1, const void *op2) {
    int avg_z1, avg_z2;
    face *f = (face*)op1, *v = (face*)op2;
    
    avg_z1 = (nt[f->a].z + nt[f->b].z + nt[f->c].z) / 3;
    avg_z2 = (nt[v->a].z + nt[v->b].z + nt[v->c].z) / 3;
    ...
    while (!kbhit()) {
        ...
        */
        ...
        for (j = 0; j < nodes; j++) {
            ...
            mul = 1 - (0.5f *(((1-cos((atan2(nt[j].x,nt[j].y) * 8) + ((float)i * 0.05f)))+
        ...
        */
        ...
        memcpy(&fz, &f, (sizeof(face) * faces));
        qsort(&fz, faces, sizeof(face), &facesort);
