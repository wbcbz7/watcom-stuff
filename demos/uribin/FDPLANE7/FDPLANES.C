...
unsigned char  texturebuf[3*65536];
unsigned char  *fdtexture, *fdpalxlat;

unsigned char  buffer[65536], fdbuffer[65536];

unsigned int   fdu[41 * 26], fdv[41 * 26], fdl[41 * 26];

unsigned char  fdblendtab[65536];
...
int main() {
    ...
    // align pointers on 64kb boundary
    fdtexture = (unsigned char*)(((int)texturebuf + 0x0FFFF) & ~0xFFFF);
    fdpalxlat = (unsigned char*)(((int)texturebuf + 0x1FFFF) & ~0xFFFF);
    ...
    for (i = 0; i < 256; i++) {
        ...
        outp(0x3C9, sat(((i >> 3) + (i >> 4) + (i >> 4)), 63));
        outp(0x3C9, sat(((i >> 3) + (i >> 3) + (i >> 4)), 63));
        outp(0x3C9, sat(((i >> 2) + (i >> 3) + (i >> 4)), 63)); 
        
    }
    for (j = 0; j < 16; j++) for (i = 0; i < 256; i++) fdpalxlat[p++] = sat(((i * j)) >> 4, 255); p = 0;
    //for (j = 0; j < 256; j++) for (i = 0; i < 256; i++) fdblendtab[p++] = sat(((i * 256) >> 8) + ((j * 64) >> 8), 255);
    for (j = 0; j < 256; j++) for (i = 0; i < 256; i++) fdblendtab[p++] = sat(((i * 192) >> 8) + ((j * 192) >> 8), 255);
